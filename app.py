...
if df.empty:
    ...
    # -----------------------------
    # Metrics (Fixed KPI Logic)
    # -----------------------------
    if "booking_status" in filtered_df.columns:
        col1, col2, col3 = st.columns(3)

        total_rides = len(filtered_df)
        completed_rides = len(filtered_df[filtered_df["booking_status"] == "Success"])
        cancelled_rides = len(filtered_df[filtered_df["booking_status"] != "Success"])
